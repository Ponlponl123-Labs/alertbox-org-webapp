    ...
    <AnimatePresence>
      {!isUnderstood && (
        <motion.div
          ...
          className="absolute h-full w-full overflow-hidden min-w-0 flex-1 backdrop-blur-sm bg-white/60 dark:bg-black/60 flex items-center justify-center p-4 z-40"
        >
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            className="absolute size-1/2 bottom-0 left-1/2 -translate-x-1/2 translate-y-1/3 rounded-full bg-linear-to-t from-indigo-800/60 to-transparent blur-[96px]"
          />
          <motion.div
